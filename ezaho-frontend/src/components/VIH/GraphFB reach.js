...
import { getFacebookReach } from "../../redux/slices/collecteVIH";
...
export default function GraphFB() {
	...
	const graphReachNow = useSelector(
		(state) => state.VIHCollecte.reach.val_current
	);
	const graphReachPrev = useSelector(
		(state) => state.VIHCollecte.reach.val_prev
	);
	...
	useEffect(() => {
		...
		dispatch(getFacebookReach(mois, yearsData));
	}, [seriesData, yearsData]);

	return (
		<Card>
			<CardHeader
				title="Couverture de la page Facebook Tanora Cool"
				action={
					<>
